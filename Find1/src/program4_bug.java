...
public class program4_bug implements Comp{
...
//				num1 -=2 -=> num1 +=2 level2
		...
		//result[5] || result[6] ==> result[5] level1
		if(result[5] ){
			num1 += 2;
		}else{
			if(result[7]){
//				num1 += 2 --> num2 *= 2 level2
		...
		if(result[8] && result[9]  || result[10] &&result[11]){
			...
			//bug result[12] || result[13] ==> result[13] level2
			if(result[13]){
				num1 += 5;
			}else{
...
//		System.out.println(Arrays.toString(node56_true[0]));
	...
	public static void main(String[] args) {
			...
			program4_bug example = new program4_bug();
